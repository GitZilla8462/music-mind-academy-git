  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(studentsRef, (snapshot) => {
      ...
      const list = Object.entries(data)
        .filter(([, s]) => (s.displayName || s.playerName || s.name) && s.tempoCharadesScore != null)
        .map(([id, s]) => ({
        id,
            ...
            {topThree[1] && (
              <div className="flex flex-col items-center">
                ...
                <div className="bg-gray-400 w-28 h-24 rounded-t-xl flex items-center justify-center mt-2">
                  <span className="text-5xl">{'\u{1F948}'}</span>
                </div>
              </div>
            )}
            ...
            {topThree[0] && (
              <div className="flex flex-col items-center">
                ...
                <div className="bg-yellow-500 w-32 h-32 rounded-t-xl flex items-center justify-center mt-2">
                  <span className="text-6xl">{'\u{1F947}'}</span>
                </div>
              </div>
            )}
...
export default TempoCharadesResults;
