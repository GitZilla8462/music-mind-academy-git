...
const SectionalLoopBuilderPresentationView = ({ sessionData }) => {
  ...
  const playSectionAudio = useCallback((section, onEnd = null) => {
    ...
    setTimeout(() => {
      ...
      // FIX: Start stop timer AFTER play begins, not before
      if (timeoutRef.current) clearTimeout(timeoutRef.current);
  ...
  const startListeningAudio = useCallback(() => {
    ...
    let positionIndex = 0;
    const FADE_DURATION = 50;   // Shorter fade for crisper transitions
    const OVERLAP = 0;          // No overlap - clean section starts
