...
const SectionalLoopBuilderResults = ({ sessionData }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(studentsRef, (snapshot) => {
      ...
      const list = Object.entries(data)
        .filter(([, s]) => (s.displayName || s.playerName || s.name) && s.score != null)
        .map(([id, s]) => ({
          id,
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-br from-green-900 via-teal-900 to-blue-900 text-white flex flex-col items-center justify-center p-8">
      {/* Title */}
