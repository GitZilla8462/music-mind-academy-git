// File: SourceOrNotStudentView.jsx
// Source or Not? News Credibility Challenge - Student View (syncs with teacher's class game)
// Students see the headline + source and vote CREDIBLE or NOT CREDIBLE
...
const SourceOrNotStudentView = ({ onComplete, isSessionMode = true }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(gameRef, (snapshot) => {
      ...
      setRoundData(data.roundData || null);
      setShownAt(data.shownAt || null);

      // Handle showing phase (new round)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(myRef, (snapshot) => {
      ...
      const newScore = data.sourceOrNotScore || 0;
  ...
  const submitAnswer = (choice) => {
    ...
    if (studentsPath && userId) {
      const db = getDatabase();
      update(ref(db, `${studentsPath}/${userId}`), {
        sourceOrNotAnswer: choice,
        sourceOrNotAnswerTime: Date.now(),
      });
    }
  };
    ...
    return (
      <div className="h-screen bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 flex items-center justify-center p-6">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-white mb-4">Source or Not?</h1>
    ...
    <div className="h-screen bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 flex flex-col p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-white/10 px-4 py-2 rounded-xl">
            <div className="text-sm text-indigo-200">{currentRound + 1}/8</div>
          </div>
        </div>
      </div>
      ...
      {roundData && (
        <div className="rounded-2xl p-5 mb-4 text-center" style={{ backgroundColor: '#1a2744', border: '2px solid rgba(240, 180, 41, 0.3)' }}>
          <div className="text-xs text-white/40 uppercase tracking-wider mb-2">Round #{currentRound + 1}</div>
          <div className="text-xl font-bold text-white leading-tight mb-3">
            &ldquo;{roundData.headline}&rdquo;
          </div>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm" style={{ backgroundColor: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)' }}>
            <span className="text-white/50">Source:</span>
            <span className="text-white font-medium">{roundData.source}</span>
          </div>
        </div>
      )}
        ...
        {gamePhase === 'showing' && !answerSubmitted && (
          <div className="w-full max-w-md space-y-4">
            <button
              ...
              style={{ backgroundColor: '#22c55e', minHeight: '80px' }}
            >
              CREDIBLE
            </button>
            <button
              onClick={() => submitAnswer('not-credible')}
              className="w-full py-8 rounded-2xl text-center transition-all hover:scale-[1.03] active:scale-95 text-white font-black text-3xl shadow-lg"
              style={{ backgroundColor: '#ef4444', minHeight: '80px' }}
            >
              NOT CREDIBLE
            </button>
          </div>
          ...
          <div className="text-center">
            <div className="bg-white/20 rounded-2xl p-8 inline-block">
              ...
              <div
                ...
                style={{ backgroundColor: selectedAnswer === 'credible' ? '#22c55e' : '#ef4444' }}
              >
                {selectedAnswer === 'credible' ? 'CREDIBLE' : 'NOT CREDIBLE'}
              </div>
        ...
        {gamePhase === 'revealed' && correctAnswer && (
          <div className="text-center w-full max-w-md">
            ...
            <div
              ...
              style={{ backgroundColor: correctAnswer === 'credible' ? '#22c55e' : '#ef4444' }}
            >
              <div className="text-lg opacity-80 mb-1">The answer is...</div>
              <div className="text-4xl font-black">
                {correctAnswer === 'credible' ? 'CREDIBLE' : 'NOT CREDIBLE'}
              </div>
            </div>

            <p className="text-indigo-200 mt-4 text-sm">Waiting for next round...</p>
          </div>
        )}
      </div>
    </div>
...
export default SourceOrNotStudentView;
