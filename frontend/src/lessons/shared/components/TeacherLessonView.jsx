...
const PresentationContent = ({
  ...
  useEffect(() => {
    ...
    // Unit 3 Music Journalist Lesson 3: Guided Listening Activity
    import('../../shared/activities/listening-guide/GuidedListeningActivity')
      .then(module => setGuidedListeningActivity(() => module.default))
      .catch(() => console.log('Guided Listening Activity not available'));
  }, []);
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-50 p-6 lg:p-12">
        ...
        <div className="flex items-center gap-8 lg:gap-16 max-w-5xl w-full">
          ...
          <div className="w-72 lg:w-80 flex-shrink-0">
            <div className="bg-white rounded-2xl shadow-lg p-5 lg:p-8 border border-gray-200 flex flex-col">
              <div className="space-y-3 lg:space-y-4 mb-4 lg:mb-5">
                <div>
                  <div className="text-gray-600 text-sm lg:text-base font-medium mb-1.5">Username</div>
                  <div className="bg-white border border-gray-300 rounded-lg px-4 lg:px-5 py-3 lg:py-4 text-gray-400 font-mono text-lg lg:text-xl">tuba123</div>
                </div>
                <div>
                  <div className="text-gray-600 text-sm lg:text-base font-medium mb-1.5">Password</div>
                  <div className="bg-white border border-gray-300 rounded-lg px-4 lg:px-5 py-3 lg:py-4 text-gray-400 font-mono text-lg lg:text-xl">epicdrum</div>
                </div>
              </div>
              <div className="bg-blue-700 text-white font-semibold py-3.5 lg:py-4 px-4 rounded-lg text-lg lg:text-xl text-center">
                Sign In
              </div>
            </div>
          </div>
  ...
  if (currentStageData?.presentationView) {
    ...
    if (type === 'dynamics-showcase') {
      ...
      return (
        <div className="absolute inset-0">
          <DynamicsShowcase sessionData={sessionData} onNextStage={goToNextStage} />
        </div>
      );
    }
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-indigo-950 via-purple-900 to-slate-900 p-8">
            ...
            <div className="flex gap-16 lg:gap-32">
              ...
              <div className="flex flex-col items-center">
                ...
                <button
                  onClick={() => playClip('crescendo', 131, 138)}
              ...
              <div className="flex flex-col items-center">
                ...
                <button
                  onClick={() => playClip('decrescendo', 64, 70)}
                  className={`px-8 py-4 rounded-xl text-xl lg:text-2xl font-bold flex items-center gap-3 transition-all ${
    ...
    if (type === 'pair-and-share-overlay') {
      ...
      return (
        <div className="absolute inset-0">
          ...
          {/* Reopen button removed — use the white Directions button on the top bar */}
        </div>
      );
    }
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-indigo-950 via-purple-900 to-slate-900 p-4 lg:p-12 overflow-hidden">
          ...
          {bullets && bullets.length > 0 && (
            <div className="max-w-7xl w-full px-6 lg:px-12">
              <ul className={isDense ? 'space-y-2 lg:space-y-4' : 'space-y-5 lg:space-y-8'}>
                {bullets.map((bullet, index) => (
                  <li key={index} className="flex items-start gap-4 lg:gap-5">
                    <span className={`text-purple-400 ${isDense ? 'text-3xl lg:text-4xl' : 'text-4xl lg:text-5xl'} mt-0.5`}>•</span>
                    <span className={`${isDense ? 'text-2xl lg:text-3xl' : 'text-3xl lg:text-5xl'} text-slate-200 leading-snug`}>{bullet}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {sections && sections.length > 0 && (
            <div className="max-w-7xl w-full px-6 lg:px-12">
              {sections.map((section, sectionIndex) => (
                <div key={sectionIndex} className={isDense ? 'mb-4 lg:mb-6' : 'mb-6 lg:mb-10'}>
                  {section.heading && (
                    <h2 className={`${isDense ? 'text-3xl lg:text-5xl mb-2 lg:mb-3' : 'text-5xl lg:text-6xl mb-3 lg:mb-6'} font-bold text-purple-400`}>
                      {section.heading}
                    </h2>
                  )}
    ...
    // Guided Listening — split-screen: teacher controls left, student worksheet right
    if (type === 'guided-listening-split') {
      const trackIndex = currentStageData.presentationView.trackIndex ?? 0;
      return (
        <div className="absolute inset-0">
          <GuidedListeningSplit
            trackIndex={trackIndex}
            GuidedListeningComponent={GuidedListeningActivity}
            onAdvanceToNext={trackIndex < 2 ? goToNextStage : null}
            nextLabel={trackIndex < 2 ? `Go to Guided Listening ${trackIndex + 2}` : null}
          />
        </div>
      );
    }
...
// ============================================
// GUIDED LISTENING SPLIT (stable component to prevent remounting)
...
const GuidedListeningSplit = React.memo(({ trackIndex, GuidedListeningComponent, onAdvanceToNext, nextLabel }) => {
  const track = GUIDED_LISTENING_TRACKS_DATA[trackIndex] || GUIDED_LISTENING_TRACKS_DATA[0];
  const [isPlaying, setIsPlaying] = useState(false);
    ...
    <div className="absolute inset-0 flex bg-white">
      ...
      <div className="w-[300px] h-full bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
        ...
        <div className="bg-gradient-to-r from-amber-600 to-orange-600 text-white px-5 py-4 shrink-0">
          ...
          <p className="text-white/70 text-sm mt-0.5">Listen &amp; analyze as a class</p>
        </div>

        {/* Directions */}
        <div className="bg-amber-50 border-b border-amber-200 px-5 py-3 shrink-0">
          <p className="font-bold text-amber-900 text-sm mb-1">Directions:</p>
          <ol className="list-decimal list-inside text-sm text-amber-800 space-y-0.5">
            <li><strong>Play</strong> the track for the class</li>
            <li>Fill out the Listening Guide <strong>together</strong></li>
            <li><strong>Discuss</strong> what you heard</li>
        ...
        <div className="flex flex-col p-4">
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-bold text-gray-900 text-lg">{track.title}</div>
            ...
            >
              {isLoading ? (
                ...
                <><Play size={22} className="ml-0.5" /> Play for Class</>
              )}
            </button>
          ...
          {/* Advance button — appears after track finishes */}
          {hasPlayed && !isPlaying && onAdvanceToNext && nextLabel && (
            <button
              onClick={onAdvanceToNext}
              className="mt-4 w-full py-3 px-4 rounded-xl bg-green-600 hover:bg-green-700 text-white font-bold text-sm transition-all flex items-center justify-center gap-2"
            >
        ...
        <div className="px-4 pb-4 shrink-0">
          <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
            <p className="text-xs text-gray-500 text-center">
              {hasPlayed && !isPlaying
                ? 'Discuss what you heard, then advance to the next track.'
                : 'Fill out the worksheet on the right as a class.'}
            </p>
          </div>
        </div>
      </div>
