...
const generatePatternLevel4 = (excludePatternIndex = -1) => {
   ...
   * 3. Must include Sol-La-Ti-Do (C5-D5-E5-F5) as four consecutive QUARTER NOTES with no rests between them
   ...
   * 8. Every pattern must start and end on Do (F4)
   ...
   * 13. Must include La (D5) at least once in each pattern
   ...
   * 17. Must include Ti (E5) at least once in each pattern
  ...
  const patterns = [
    // Pattern 1 - Following ALL Level 4 rules including the new Do-Sol skip requirement
    {
      notes: [
        ...
        { pitch: 'E5', syllable: 'Ti', duration: 'h' },
    ...
    // Pattern 3 - Third exercise ending on half note Do
    {
      notes: [
        ...
        { pitch: 'F4', syllable: 'Do', duration: 'q' },
        { type: 'rest', duration: 'q' },
        { pitch: 'F4', syllable: 'Sol', duration: 'h' },
...
const Lvl4SolfegeID2 = ({ onClose }) => {
  const config = {
    title: 'Level 4 Solfege Identification',
    subtitle: 'Solfege: Do-Do • Rhythm: Quarter, Half, and Eighth Notes • Quarter Rests • Key: F Major • Dynamics: f/p',
    ...
    getLinePosition: (pitch, stave) => {
      switch(pitch) {
        ...
        case 'e/5': return stave.getYForLine(2);
        case 'f/5': return stave.getYForLine(1.5);
