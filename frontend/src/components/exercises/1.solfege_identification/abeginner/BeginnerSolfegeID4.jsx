// /src/components/exercises/SolfegeExerciseFour.jsx - Do-Re-Mi-Fa-Sol Exercise
...
// Exercise 4: Do-Re-Mi-Fa-Sol Configuration
...
// Generate stepwise pattern - always start on Do, stepwise motion, include all syllables
const generatePattern = () => {
  ...
  // Keep generating until we have at least one of each syllable
  while (attempts < maxAttempts) {
    pattern = [];
    let currentNoteIndex = 0; // Always start on Do
    ...
    // Check if pattern includes at least one of each syllable
    ...
    if (hasAllSyllables) {
      break; // Pattern is valid, exit loop
    }
    
    attempts++;
  }
  
  // If we couldn't generate a valid pattern, force include all syllables
  if (attempts >= maxAttempts) {
    console.log('Forcing pattern with all syllables...');
    pattern = [
      { pitch: 'C4', syllable: 'Do' },
      { pitch: 'D4', syllable: 'Re' },
      { pitch: 'E4', syllable: 'Mi' },
      { pitch: 'F4', syllable: 'Fa' },
      { pitch: 'G4', syllable: 'Sol' },
      { pitch: 'F4', syllable: 'Fa' },
      { pitch: 'E4', syllable: 'Mi' },
      { pitch: 'D4', syllable: 'Re' }
    ];
...
// Solfege Buttons Component - Fixed version for Exercise 4
      ...
      {syllables.map((syllable) => {
        ...
        return (
          <button
            ...
            disabled={disabled}
            className={`px-6 py-4 text-xl font-bold border-2 rounded-lg transition-all duration-200 transform hover:scale-105 ${buttonStyle} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            {syllable}
          </button>
        );
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-md mx-auto">
        ...
        <div className="space-y-3">
          <div className="flex justify-between">
            ...
            <span className="font-medium">Do-Re-Mi-Fa-Sol Identification</span>
          </div>
          <div className="flex justify-between">
...
const SolfegeExerciseFour = ({ onClose }) => {
    ...
    const initVocalAudio = async () => {
      try {
        ...
        const audio = {
          ...
          'Mi': new Audio('/teacher_dashboard/sounds/mi.mp3'),
          'Fa': new Audio('/teacher_dashboard/sounds/fa.mp3'),
          'Sol': new Audio('/teacher_dashboard/sounds/sol.mp3') // NEW: Sol audio file
        };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-800">
            Exercise 4: {EXERCISE_CONFIG.title}
          </h1>
...
export default SolfegeExerciseFour;